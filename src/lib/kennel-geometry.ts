...
const ISO_X = -0.13;
const ISO_Y = 0.8;
const DEPTH_SKEW = 0.08;
const FRONT_SPREAD = 0.22;
const makeDims = (W: number, D: number, h: number): Dims => ({ W, D, h, ex: [W, W * ISO_X], ey: [-D * DEPTH_SKEW, D * ISO_Y] });
...
function corners(d: Dims, BL: Pt) {
  ...
  const FL: Pt = [BL[0] + d.ey[0] - d.W * FRONT_SPREAD, BL[1] + d.ey[1]];
  const FR: Pt = [BL[0] + d.ex[0] + d.ey[0] + d.W * FRONT_SPREAD, BL[1] + d.ex[1] + d.ey[1]];
  return {
    FL, BL, BR, FR, h: d.h,
    /** u runs along the width edge, v along the depth edge; both 0..1. */
    floorAt: (u: number, v: number) => ({
      x: BL[0] + d.ex[0] * u + d.ey[0] * v + d.W * FRONT_SPREAD * v * (2 * u - 1),
...
export function kennelGeometry(spec: BoxSpec, withYard: boolean) {
  ...
  const pad = 16;
  const vw = canvas.W * (1 + 2 * FRONT_SPREAD) + canvas.D * DEPTH_SKEW + pad * 2;
  const vh = canvas.W * Math.abs(ISO_X) + canvas.D * ISO_Y + canvas.h + pad * 2;
  const canvasBL: Pt = [canvas.D * DEPTH_SKEW + canvas.W * FRONT_SPREAD + pad, canvas.h + pad + canvas.W * Math.abs(ISO_X)];
  const centre: Pt = [
